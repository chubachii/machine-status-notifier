...
def plot_box(image, top_left_point, bottom_right_point, width, height, label, color=(210,240,0), padding=6, font_scale=3):
    label = label.upper()
    if label[0] == 'Y': color=(0, 240, 255)
    elif label[0] == 'R': color=(0, 0, 255)
    ...
    p3 = top_left_point['x'] + font_width + padding*2, top_left_point['y'] - font_height - padding*2 - 20
    cv2.rectangle(image, (top_left_point['x'] - 2, top_left_point['y']-20), p3, color, -1, lineType=cv2.LINE_AA)
    x = top_left_point['x'] + padding
    y = top_left_point['y'] - padding
    cv2.putText(image, label, (x, y-20), font_face, font_scale, [0, 0, 0], thickness=1, lineType=cv2.LINE_AA)
    return image
